...
using tuplaCarro = std::tuple<int, int, bool>; // (motorista, horário,
...
struct Comparador {
  bool operator()(const tuplaCarro &a, const tuplaCarro &b) {
    if (std::get<1>(a) == std::get<1>(b)) {
      return std::get<2>(a) >
             std::get<2>(
                 b); // Saídas têm prioridade em caso de empate no horário
    }
    return std::get<1>(a) > std::get<1>(b); // Ordem crescente por horário
  }
};
using estacionamento =
    std::priority_queue<tuplaCarro, std::vector<tuplaCarro>, Comparador>;
